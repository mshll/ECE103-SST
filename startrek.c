/**
 * @file  startrek.c
 * @brief ECE103 Final Project: Super Star Trek Game
 *
 * @author  Meshal Almutairi (meshal7@pdx.edu)
 * @date    2021-June-10
 *
 */
...
// Structs & Enums
typedef enum conditions_t { green,
                            yellow,
                            red,
                            docked } Condition;
enum status_t { warp,
...
World generateWorld() {
  ...
  World world = {
      ...
      .player = {
          ...
          .photon = 10,
          .pos = {4, 5, 3, 0}},
      .statNames = {"WARP ENGINES", "SHORT RANGE SENSORS", "LONG RANGE SENSORS", "PHASER CONTROL", "PHOTON TUBES", "DAMAGE CONTROL", "SHIELD CONTROL", "LIBRARY-COMPUTER"}};
...
void updateCond(World *world) {
  ...
  if (world->quadrant[q1][q2].numStarbases > 0)
    world->player.condition = docked;
  else if (world->quadrant[q1][q2].numKlingons > 0)
    world->player.condition = red;
  else if (world->player.energy + world->player.shield < (PLAYER_ENERGY * 0.1))
    world->player.condition = yellow;
}
...
void getCmd(World *world) {
  ...
  if (!strncmp(input, "NAV", 3))
    cmdNAV(world);
  else if (!strncmp(input, "SRS", 3))
    cmdSRS(world);
  else if (!strncmp(input, "LRS", 3))
    cmdLRS(world);
  else if (!strncmp(input, "PHA", 3))
    cmdPHA(world);
  else if (!strncmp(input, "TOR", 3))
    cmdTOR(world);
  else if (!strncmp(input, "SHE", 3))
    cmdSHE(world);
  else if (!strncmp(input, "DAM", 3))
    cmdDAM(world);
  else if (!strncmp(input, "COM", 3))
    cmdCOM(world);
  else if (!strncmp(input, "XXX", 3))
    cmdXXX(world);
  else if (!strncmp(input, "INS", 3))
    printInstructions();
  else {
    printf(
...
void printStat(World *world, int n) {
  ...
  switch (n) {
    ...
    case 1: {
      char cond[7] = "GREEN";
      if (world->player.condition == yellow)
        strcpy(cond, "*YELLOW*");
      else if (world->player.condition == red)
        strcpy(cond, "*RED*");
      else if (world->player.condition == docked)
        strcpy(cond, "DOCKED");
      printf("CONDITION:           %s\n", cond);
      break;
    }
...
void cmdSRS(World *world) {
  ...
  for (int s1 = 0; s1 < QS_SIZE; s1++) {
    ...
    for (int i = 0; i < 3; i++) {
      if (world->quadrant[q1][q2].sector[s1][0] == 'K' && i == 2)
        printf("+");
      else if (world->quadrant[q1][q2].sector[s1][0] == 'E' && i == 2)
        printf("<");
      else if (world->quadrant[q1][q2].sector[s1][0] == 'B' && i == 2)
        printf(">");
      else
        printf(" ");
    }
    ...
    for (int s2 = 0; s2 < QS_SIZE; s2++) {
      ...
      for (int i = 0; i < 3; i++) {
        if ((world->quadrant[q1][q2].sector[s1][s2] == 'K' && i == 0) || (world->quadrant[q1][q2].sector[s1][s2 + 1] == 'K' && i == 2 && s2 < QS_SIZE - 1))
          printf("+");
        else if ((world->quadrant[q1][q2].sector[s1][s2] == 'E' && i == 0) || (world->quadrant[q1][q2].sector[s1][s2 + 1] == 'B' && i == 2 && s2 < QS_SIZE - 1))
          printf(">");
        else if ((world->quadrant[q1][q2].sector[s1][s2 + 1] == 'E' && i == 2 && s2 < QS_SIZE - 1) || (world->quadrant[q1][q2].sector[s1][s2] == 'B' && i == 0))
          printf("<");
        else
          printf(" ");
      }
    ...
    if (numCOM == 0) {
      ...
      for (int r = 0; r < QS_SIZE; r++) {
        ...
        for (int c = 0; c < QS_SIZE; c++) {
          ...
          if (world->quadrantArch[r][c].scanned)
            printf("%d%d%d   ", quad->numKlingons, quad->numStarbases, quad->numStars);
          else
            printf("***   ");
        }
        printf("\n     ----- ----- ----- ----- ----- ----- ----- -----\n");
